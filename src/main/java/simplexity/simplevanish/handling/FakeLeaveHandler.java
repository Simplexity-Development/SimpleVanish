...
public class FakeLeaveHandler {
    ...
    public void sendFakeLeaveMessage(@NotNull Player player) {
        Component message;
        if (ConfigHandler.getInstance().isCustomJoinLeave()) {
            message = MessageHandler.parsePlayerMessage(player, ConfigHandler.getInstance().getCustomLeaveMessage());
        } else {
